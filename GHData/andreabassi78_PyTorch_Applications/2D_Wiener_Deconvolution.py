...
def wiener2D(signal, kernel, snr):
    """
    (Depthwise) Deconvolution using the Wiener algorithm for Pytorch.
    ...
    snr: the expected snr of the image 

    Returns
    -------
    deconvolved signal, same size as signal
    ...
    padding = ( (Y-psfY)//2, (Y-psfY)//2+1 , (X-psfX)//2, (X-psfX)//2+1)
    
    kernel_pad = torch.nn.functional.pad(kernel,padding)
    ...
    deconvolved = fftshift( torch.real(torch.fft.ifftn( 
        (torch.fft.fftn(signal, dim=(2,3))*torch.conj(H))/ (H*torch.conj(H) + snr**2), dim=(2,3)
                                                                )
                                                ),dim=(2,3))
...
if __name__ == '__main__':
    ...
    t2 = time.time()
    restored = wiener2D(out, psf, 0.05)
