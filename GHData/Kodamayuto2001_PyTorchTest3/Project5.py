...
class MyDataset:
    def __init__(self,pathImgTrim,tagName):
        try:
            ...
            print("指定されたファイルのパスが見つかりません。\nパスの確認をお願いします。もしデータを作っていないのなら、先にMyDatasetクラスのMakeData関数を読んでデータを作ってください。")
        try:
            self.__setTag(tagName)
        except:
            print("タグの名前が読み取れませんでした。")
        try:
            self.__ndarrayTransform()
        except AttributeError:
            print("パスの中にデータがありませんでした。パスの確認とデータ作成お願いします。")
        try:
            self.__CSV_transform()
        except:
            print("CSVファイルに変換できませんでした。")
        try:
            self.__setImgSize()
        except:
            print("ファイルのサイズを変更できませんでした。")
        try:
            self.__Rescale()
        except:
            print("正しく拡大縮小できませんでした。")
        print("-----処理終了-----")

    def MakeData(self):
        print("データを作成するディレクトリのパスを入力してください。")
        self.__dirpath = input("ディレクトリのパス：")
        try:
            self.__MyMakeData()
        except cv2.error:
            print("このファイルと同じディレクトリにhaarlike特徴分類器をダウンロードしてください。https://github.com/opencv/opencvにアクセス")
            print("わからないことがありましたら管理者に聞いてください。")
    
    def __MyMakeData(self):
        ...
        while True:
            ...
            pathImage = self.__dirpath+"Resources/Images/"+str(cnt)+".jpg"
            pathImgTrim = self.__dirpath+"Resources/ImgTrim/"+str(cnt)+".jpg"
