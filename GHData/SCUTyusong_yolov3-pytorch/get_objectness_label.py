...
def get_objectness_label(img, gt_boxes, gt_labels, iou_threshold=0.7,
    ...
    num_anchors = len(anchors) // 2  # 共3个锚框
    input_h = img_shape[2]
    input_w = img_shape[3]
    ...
    for n in range(batchsize):
        ...
        for n_gt in range(len(gt_boxes[n])):
            gt = gt_boxes[n][n_gt]  # 当前图片的真实框
            gt_cls = gt_labels[n][n_gt]
            gt_center_x = gt[0]
            gt_center_y = gt[1]
            gt_width = gt[2]
            gt_height = gt[3]
            if (gt_height < 1e-3) or (gt_width < 1e-3):
                continue
            i = int(gt_center_y * num_rows)  # 锚框中心的真实坐标
            j = int(gt_center_x * num_cols)
            ious = []
            for ka in range(num_anchors):  # num_anchors is 3·
                bbox1 = [0., 0., float(gt_width), float(gt_height)]
                anchor_w = anchors[ka * 2]
                anchor_h = anchors[ka * 2 + 1]
                bbox2 = [0., 0., anchor_w / float(input_w), anchor_h / float(input_h)]
                # 计算iou bbox1 truth bbox2 anchors
                iou = box_iou_xywh(bbox1, bbox2)
                ious.append(iou)
            ious = np.array(ious)
            inds = np.argsort(ious) # from small to big [ 2 1 0]
            k = inds[-1]
            label_objectness[n, k, i, j] = 1  # 将iou最大的那个框设置标签为1
            c = gt_cls
    ...
    # 注意，这里并没用解决label为-1的预测框问题，后面建立起损失函数之后再补上
