...
def lstm_model_fn(input_x, vocab_size, embed_length):
	...
	# dense = tf.layers.dense(inputs=last, units=2)
	dense = tf.layers.dense(inputs=last, units=2, activation=tf.nn.relu)
...
def main():
	...
	seq_length = 30
	embed_length = 15
	train_data, test_data, vocab_size = load_text_dataset('datasets/sentiment/', seq_length)
