...
def load_data_file(txt_filename, target_filename):
    txt = open(txt_filename, 'r')
    target = open(target_filename, 'r')
    X,Y = [],[]
    for txt_line, target_line in zip(txt, target):
        X.append(txt_line.strip())
        Y.append(target_line.strip())
    return X, Y
...
class ProcessData(object):
    def __init__(self, pretrain_wv=None, lower=True, min_df=5):
        ...
        if pretrain_wv is not None:
            ...
            self.wv = None
        self.embs = [np.zeros((300,)), np.random.random((300,))*.01]
