...
def training_loop():
    ...
    dataloader_sketch= create_dataloader(opt.dataroot_sketch,
                                        opt.size,
                                        opt.batch)    
    
    # dataloader for image regularization
    if opt.dataroot_image is not None:#default=None
        dataloader_image = create_dataloader(opt.dataroot_image,
                                            opt.size,
                                            opt.batch)
        data_yield_image=iter(dataloader_image)
    ...
    for epoch in range(opt.max_epoch):
        ...
        for i, (data_sketch,label) in enumerate(dataloader_sketch):  # inner loop within one epoch
            ...
            for i in range(opt.iter_per_epoch):
                if opt.dataroot_image is not None:
                    try:
                        data_image,label2 = next(data_yield_image)
