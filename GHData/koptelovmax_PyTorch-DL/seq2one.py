...
class RNN(nn.Module):
    ...
    def forward(self, x):
        x = self.rnn(x)[0] # output features (ignore hidden state)
        x = self.f(x)
        return x
...
difficulty = QRSU.DifficultyLevel.EASY # EASY, NORMAL, MODERATE or HARD
batch_size = 32
...
hidden_size = 8 # 64 for DifficultyLevel.NORMAL
