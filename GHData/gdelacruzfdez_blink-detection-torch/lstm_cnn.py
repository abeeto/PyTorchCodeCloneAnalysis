...
class LSTMModel(ABC):
    ...
    def __initialize_cnn_model(self):
        self.cnn_model = network.Resnet(extract=True)
        self.cnn_model.load_state_dict(torch.load(self.cnn_model_file), strict=False)
        self.cnn_model.eval()
        if self.cuda:
    ...
    def __initialize_training_parameters(self):
        ...
        #    self.optimizer, 'min', patience=8, factor=0.1, verbose=True)
    ...
    def process_samples_through_cnn(self, samples, targets):
        features = self.cnn_model.forward(samples)
    ...
    def fit(self):
        for epoch in range(1, self.epochs + 1):
            ...
            self.current_train_loss = train_stats['loss']
            ...
            torch.save(self.lstm_model.state_dict(), 'last_model.pt')
...
class EyeStateDetectionSingleInputLSTMModel(LSTMModel):
...
def create_lstm_model(params, cuda):
    ...
    if 'BLINK_DETECTION_MODE' == eval_mode:
        ...
        return EyeStateDetectionSingleInputLSTMModel(params, cuda)
    else:
