...
class LSTMModel(ABC):
    ...
    def initialize_lstm_model(self):
        self.lstm_model = network.BiRNN(
            self.dims, self.lstm_hidden_units, self.lstm_layers, self.num_classes, bidirectional=self.bidirectional)
        print(summary(self.lstm_model, input_size=(self.sequence_len,1,self.dims)))
        if self.cuda:
    ...
    def __train_epoch(self, epoch):
        ...
        for batch_idx, data in progress:
            ...
            precision, recall, f1, _= metrics.precision_recall_fscore_support(all_targets, predictions, average="macro")
