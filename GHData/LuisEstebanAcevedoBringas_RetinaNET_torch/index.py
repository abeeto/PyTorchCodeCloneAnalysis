from RetinaNet import  RetinaNET_TransferLearning, RetinaNET_FineTuning, RetinaNET_FromScratch
from create_dataset import VOCDataset,Dataset_MM
from transformation import get_transform
from engine import train_one_epoch
from RetinaNET_utils import collate_fn
import torch
import time

def save_model(epoch, model, optim, name = None):
    if name is None:
        filename = '/home/fp/Escritorio/LuisBringas/RetinaNET/checkpoints/MM_Dataset/Checkpoint_FineTuning_MM_Dataset.pth.rar'
    else:
        filename = '/home/fp/Escritorio/LuisBringas/RetinaNET/checkpoints/MM_Dataset/{}.pth.rar'.format(name)
...
def main(checkpoint = None):
    ...
    dataset = Dataset_MM('/home/fp/Escritorio/LuisBringas/FCOS/JSONfiles_G', 'TRAIN', get_transform(True)) #Mask Dataset
    ...
    if checkpoint is None:
        start_epoch = 1
        #model = RetinaNET_TransferLearning(num_classes) #LLamamos la funcion para cambiar el metodo de entrenamiento a TL
        model = RetinaNET_FineTuning(num_classes) #LLamamos la funcion para cambiar el metodo de entrenamiento a FT
        #model = RetinaNET_FromScratch(num_classes) #LLamamos la funcion para cambiar el metodo de entrenamiento a FS
        model.to(device)
