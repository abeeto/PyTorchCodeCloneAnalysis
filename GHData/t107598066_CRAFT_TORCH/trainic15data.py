...
if __name__ == '__main__':
    print("load data Syntext")
    dataloader = Synth80k('../ICDAR/SynthText/', target_size = 768)
    ...
    model = CRAFT()
    print("load Syntext model")
    model.load_state_dict(copyStateDict(torch.load('../pth/backtime92/Syndata.pth')))
    ...
    model.train()
    print("load data ICDAR2015")
    realdata = ICDAR2015(model, '../ICDAR/2015/train/', target_size=768)
    ...
    for epoch in range(1):
        ...
        torch.save(model.state_dict(),
                   save_path + 'CRAFT_clr_' + repr(epoch) + '.pth')
        test(save_path + 'CRAFT_clr_' + repr(epoch) + '.pth')
