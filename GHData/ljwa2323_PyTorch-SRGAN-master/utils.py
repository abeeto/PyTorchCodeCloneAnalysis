...
def normalize(x):
    ...
    mi = np.min(x, axis=(2, 3), keepdims=True)
    ma = np.max(x, axis=(2, 3), keepdims=True)

    return (x - mi) / (ma - mi + 0.000001) * 255.0 , mi, ma

def rev_normalize(x, mi, ma):

    assert len(x.shape) == 4

    return (x/255.0 + mi) * (ma - mi + 0.000001)
