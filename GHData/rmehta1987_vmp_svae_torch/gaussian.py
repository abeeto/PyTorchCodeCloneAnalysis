...
eps = torch.tensor(1e-10).to('cuda')


def natural_to_standard(eta1, eta2):
    """ Convert natural parameters to standard normal parameters mu and sigma
        Args: 
            eta1: natural parameter one of normal distribution mu*precision, requires_grad = True
            eta2: natural parameter two of normal distribution -0.5*precision, requires_grad = True
...
def standard_to_natural(mu, sigma):
    """
    Args:
        mu: Num Components x Latent Dims
        sigma: Num Components x Latent Dims x Latent Dims
    """
...
def log_probability_nat(x, eta1, eta2, weights=None):
    ...
    # Change 2 to a variable, or pass in dimension as int, currenlty doing this because JIT breaks
    logprob += -0.5 * logdet(-2. * eta2 + eps + torch.eye(D).to('cuda'))

    if weights is not None: # Need to also make sure weights are not negative
        #logprob += torch.log(weights).unsqueeze(0) since using log_softmax instead of softmax we don't need to take log of weights
        logprob += weights.unsqueeze(0)
    
    # log sum exp trick [log (sum(e^x_i))] to normalize log probabilities
