...
class Decoder(nn.Module):
    ...
    def initalize(self):
        ...
        for i, (hidden_units, actlayer) in enumerate(self.layers):
            ...
            elif i == len(self.layers)-1: # For the output layer (and last layer) that has a mean, variance (or for natural parameters eta1 and eta2)
                modules.append(nn.Linear(prev_units,self.output_dim*2))
                modules.append(the_act_layer)
            else:
...
class Standard_Activation(nn.Module):
    ...
    def forward(self, input):
        ...
        mean = raw_1
        self.stan_soft = torch.nn.Softplus()
        var = self.stan_soft(raw_2)
        return mean, var
