...
def conv3x3(in_planes, out_planes, M, nScale, stride=1, groups=1, dilation=1):
...
def conv1x1(in_planes, out_planes, M, nScale, stride=1):
    """1x1 convolution"""
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, M, nScale, stride=1, downsample=None, groups=1,
        ...
        self.conv1 = conv3x3(inplanes, planes, M, nScale, stride)
        self.bn1 = norm_layer(planes)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = conv3x3(planes, planes, M, nScale)
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, M, nScale, GCN_is_maxpool, num_classes=1000, zero_init_residual=False,
        ...
        self.layer2 = self._make_layer(block, 128, layers[1], M, nScale, stride=2,
                                       dilate=replace_stride_with_dilation[0])
        self.layer3 = self._make_layer(block, 256, layers[2], M, nScale, stride=2,
                                       dilate=replace_stride_with_dilation[1])
        self.layer4 = self._make_layer(block, 512, layers[3], M, nScale, stride=2,
    ...
    def _make_layer(self, block, planes, blocks, M, nScale, stride=1, dilate=False):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                conv1x1(self.inplanes, planes * block.expansion, M, nScale, stride),
                norm_layer(planes * block.expansion),
            )

        layers = []
        layers.append(block(self.inplanes, planes, M, nScale, stride, downsample, self.groups,
                            self.base_width, previous_dilation, norm_layer))
        self.inplanes = planes * block.expansion
        for _ in range(1, blocks):
            layers.append(block(self.inplanes, planes, M, nScale, groups=self.groups,
