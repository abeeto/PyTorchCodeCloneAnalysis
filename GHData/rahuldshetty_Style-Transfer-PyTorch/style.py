...
def get_style_model_and_losses(cnn,norm_mean,norm_std,style_img,content_img,content_layers=content_layers_default,style_layers=style_layers_default):
	...
	for layer in cnn.children():
		if isinstance(layer,nn.Conv2d):
			...
			raise Exception("Unknown layer:{}".format(layer.__class__.__name__))
...
def run_style_transfer(cnn,norm_mean,norm_std,content_img,style_img,input_img,num_steps=500,style_weight=1000000,content_weight=1):
...
plt.figure()
imshow(output,title='Outpt Image')
plt.ioff()
plt.show()
