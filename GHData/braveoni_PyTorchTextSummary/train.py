    ...
    for epoch_count in range(1 + past_epochs, params.n_epochs + 1):
        ...
        for batch_count in prog_bar:  # training batches
            if params.forcing_decay_type:
                if params.forcing_decay_type == 'linear':
                    forcing_ratio = max(
                        0, params.forcing_ratio - params.forcing_decay * total_batch_count)
            ...
            if metric is not None:  # print ROUGE as well if reinforcement learning is enabled
                ...
                prog_bar.set_postfix(
                    loss='%g' % epoch_avg_loss, rouge='%.4g' % (epoch_avg_metric * 100))
            else:
        ...
        if valid_generator is not None:  # validation batches
            ...
            for batch_count in prog_bar:
                ...
                loss = eval_batch(batch, model, vocab, criterion, pack_seq=params.pack_seq,
                                          show_cover_loss=params.show_cover_loss)
                valid_loss += loss
...
if __name__ == "__main__":
    ...
    if args.resume_from:
        ...
        m = torch.load('%s.%02d.pt' %
                       (args.resume_from[:-9], train_status['epoch']))
    ...
    train_gen = dataset.generator(
        p.batch_size, v, v, True if p.pointer else False)
    if p.val_data_path:
