...
class Vocab(object):
    ...
    def trim(self, *, vocab_size: int = None, min_freq: int = 1):
        if min_freq <= 1 and (vocab_size is None or vocab_size >= len(self.word2idx)):
            return 
    ...
    def load_embeddings(self, dtype=np.float32):
        num_embeddings = 0
        vocab_size = len(self)
        navec = Navec.load('data/embeddings/navec_hudlit_v1_12B_500K_300d_100q.tar')
        ...
        for word in navec.vocab.words:
            ...
            if idx is not None:
                ...
                if self.embeddings is None:
                    n_dims = len(vec)
                    self.embeddings = np.random.normal(np.zeros((vocab_size, n_dims))).astype(np.float32)
    ...
    def is_word(self, token_id: int) -> bool:
        ...
        token_str = self.idx2word[token_id]
...
class OOVDict(object):
  ...
  def add_word(self, idx_in_batch, word) -> int:
    ...
    self.idx2word[(idx_in_batch, index)] = word
...
class Dataset(object):
    ...
    def simple_tokenizer(self, text: str, lower: bool=False, newline: str=None) -> list[str]:
        """Split an already tokenized input `text`."""
        if lower:
            text = text.lower()
        if newline is not None:  # replace newline by a token
            text = text.replace('\n', ' ' + newline + ' ')
        return [word.text.lower() for word in self.nlp.tokenizer(text) if word.text not in punctuation]

    def __init__(self, filename: str, max_src_len: int=None,
        ...
        for text, summary in zip(df['text'], df['summary']):
            src = self.simple_tokenizer(text)
            ...
            tgt = self.simple_tokenizer(summary)
    ...
    def build_vocab(self, vocab_size: int = None, src: bool = True, tgt: bool = True, embed_file: str = None) -> Vocab:
                ...
                for i, example in enumerate(examples):
                    ...
                    if tgt_vocab:
                        for j, word in enumerate(example.tgt):
                            idx = tgt_vocab[word]
                            if ext_vocab and idx == src_vocab.UNK:
                                idx = oov_dict.word2idx.get((i, word), idx)
                            tgt_tensor[j, i] = idx
