...
class G(nn.Module):
    def __init__(self, input_nc, output_nc, ngf):
        ...
        self.batch_norm3 = nn.BatchNorm2d(ngf * 8)
        self.batch_norm4 = nn.BatchNorm2d(ngf * 8)
        self.batch_norm5 = nn.BatchNorm2d(ngf * 8)
        self.batch_norm6 = nn.BatchNorm2d(ngf * 8)
        self.batch_norm7 = nn.BatchNorm2d(ngf * 8)
        self.batch_norm8 = nn.BatchNorm2d(ngf * 8)
        self.batch_norm9 = nn.BatchNorm2d(ngf * 8)
        self.batch_norm10 = nn.BatchNorm2d(ngf * 8)
        self.batch_norm11 = nn.BatchNorm2d(ngf * 4)
        self.batch_norm12 = nn.BatchNorm2d(ngf * 2)
        self.batch_norm13 = nn.BatchNorm2d(ngf)
    ...
    def forward(self, input):
        ...
        e2 = self.batch_norm1(self.conv2(self.leaky_relu(e1)))
        # state size is (ngf x 2) x 64 x 64
        e3 = self.batch_norm2(self.conv3(self.leaky_relu(e2)))
        # state size is (ngf x 4) x 32 x 32
        e4 = self.batch_norm3(self.conv4(self.leaky_relu(e3)))
        # state size is (ngf x 8) x 16 x 16
        e5 = self.batch_norm4(self.conv5(self.leaky_relu(e4)))
        # state size is (ngf x 8) x 8 x 8
        e6 = self.batch_norm5(self.conv6(self.leaky_relu(e5)))
        # state size is (ngf x 8) x 4 x 4
        e7 = self.batch_norm6(self.conv7(self.leaky_relu(e6)))
        ...
        d1_ = self.dropout1(self.batch_norm7(self.dconv1(self.relu(e8))))
        # state size is (ngf x 8) x 2 x 2
        d1 = torch.cat((d1_, e7), 1)
        d2_ = self.dropout2(self.batch_norm8(self.dconv2(self.relu(d1))))
        # state size is (ngf x 8) x 4 x 4
        d2 = torch.cat((d2_, e6), 1)
        d3_ = self.dropout3(self.batch_norm9(self.dconv3(self.relu(d2))))
        # state size is (ngf x 8) x 8 x 8
        d3 = torch.cat((d3_, e5), 1)
        d4_ = self.batch_norm10(self.dconv4(self.relu(d3)))
        # state size is (ngf x 8) x 16 x 16
        d4 = torch.cat((d4_, e4), 1)
        d5_ = self.batch_norm11(self.dconv5(self.relu(d4)))
        # state size is (ngf x 4) x 32 x 32
        d5 = torch.cat((d5_, e3), 1)
        d6_ = self.batch_norm12(self.dconv6(self.relu(d5)))
        # state size is (ngf x 2) x 64 x 64
        d6 = torch.cat((d6_, e2), 1)
        d7_ = self.batch_norm13(self.dconv7(self.relu(d6)))
        # state size is (ngf) x 128 x 128
