...
def main():
    ...
    liif_model, ema_liif_model = build_model()
    print(f"Build `{config.model_arch_name}` model successfully.")

    criterion = define_loss()
    print("Define all loss functions successfully.")

    optimizer = define_optimizer(liif_model)
    ...
    if config.pretrained_model_weights_path:
        liif_model = load_state_dict(liif_model, config.pretrained_model_weights_path)
    ...
    if config.resume_model_weights_path:
        liif_model, ema_liif_model, start_epoch, best_psnr, best_ssim, optimizer, scheduler = load_state_dict(
            liif_model,
    ...
    for epoch in range(start_epoch, config.epochs):
        train(liif_model,
              ...
              scaler,
              writer)
        psnr, ssim = validate(liif_model, test_prefetcher, epoch, writer, psnr_model, ssim_model, "Test")
        print("\n")
        ...
        save_checkpoint({"epoch": epoch + 1,
                         "best_psnr": best_psnr,
                         "best_ssim": best_ssim,
                         "state_dict": liif_model.state_dict(),
                         "ema_state_dict": ema_liif_model.state_dict(),
...
def build_model() -> [nn.Module, nn.Module]:
    liif_model = model.__dict__[config.model_arch_name](in_channels=config.in_channels,
                                                        encoder_channels=config.encoder_channels,
                                                        out_channels=config.out_channels)
    liif_model = liif_model.to(device=config.device)
    ...
    ema_liif_model = AveragedModel(liif_model, avg_fn=ema_avg)

    return liif_model, ema_liif_model
...
def define_optimizer(liif_model) -> optim.Adam:
    optimizer = optim.Adam(liif_model.parameters(),
                           config.model_lr,
                           config.model_betas,
                           config.model_eps,
...
def train(
        liif_model: nn.Module,
        ema_liif_model: nn.Module,
    ...
    while batch_data is not None:
        ...
        liif_model.zero_grad(set_to_none=True)

        # Mixed precision training
        with amp.autocast():
            sr = liif_model(lr, coord, cell)
        ...
        # Update EMA
        ema_liif_model.update_parameters(liif_model)
