...
class ValueNetwork(nn.Module):
    def __init__(self, num_inputs, num_actions, hidden_size1=128,hidden_size2=256,hidden_size3=512,hidden_size4=32, init_w=3e-3):
        ...
        self.act_layer = Sequential(nn.Linear(num_actions, 512),nn.LeakyReLU(.2),nn.BatchNorm1d())
        self.state_layer = Sequential(nn.Linear(num_inputs, 512),nn.LeakyReLU(.2),nn.BatchNorm1d())
        ...
        self.hidden0 = Sequential(nn.Linear(1024, 512), nn.LeakyReLU(.2), nn.BatchNorm1d())
        self.hidden1 = Sequential(nn.Linear(512, 256), nn.LeakyReLU(.2), nn.BatchNorm1d())
        self.hidden2 = Sequential(nn.Linear(256, 64), nn.LeakyReLU(.2), nn.BatchNorm1d())
        self.hidden3 = Sequential(nn.Linear(64, 32), nn.LeakyReLU(.2), nn.BatchNorm1d())
        self.out = Sequential(nn.Linear(32, 1))
    ...
    def forward(self, state, action):
        ...
        x1 = self.act_layer(action)
        x2 = self.state_layer(state)
        x = self.hidden0(torch.cat([x1, x2],1))
        x= self.hidden1(x)
        x= self.hidden2(x)
        x = self.hidden3(x)
        x = self.out(x)
...
class PolicyNetwork(nn.Module):
    def __init__(self, num_inputs, num_actions, hidden_size, init_w=3e-3):
        ...
        self.hidden0 = Sequential(nn.Linear(num_inputs,1024),nn.LeakyReLU(.2),nn.BatchNorm1d())
        self.hidden1 = Sequential(nn.Linear(1024,512),nn.LeakyReLU(.2),nn.BatchNorm1d())
        self.hidden2 = Sequential(nn.Linear(512,256),nn.LeakyReLU(.2),nn.BatchNorm1d())
        self.hidden3 = Sequential(nn.Linear(256, 64), nn.LeakyReLU(.2), nn.BatchNorm1d())
        self.out = Sequential(nn.Linear(64, num_actions), nn.Sigmoid())
