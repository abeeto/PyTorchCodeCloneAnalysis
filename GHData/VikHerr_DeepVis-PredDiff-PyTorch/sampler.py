...
class ConditionalSampler:
    ...
    def get_samples(self, sampleIndices, featVect_orig, numSamples=100):
        ...
        # remove batch idx from shape
        b,c,x,y = featVect.shape
        f_shape = (c,x,y)     
        # get a patch surrounding the sample indices and the indices relative to that
        patch, patchIndices = self._get_surr_patch(featVect.reshape(f_shape), sampleIndices)
