...
def load_and_transform(imagePath, max_size = 512, shape = None):
...
def style_transfer(targetImage,styleGrams,contentFeatures,show = 5,alpha = 8,
                   beta = 1e4,steps = 2000,
                   styleWeights = {'conv1_1': 1.,'conv2_1': 0.8,'conv3_1': 0.5,
                   'conv4_1': 0.3,'conv5_1': 0.1},learning_rate = 1):
    
    # Define optimizer
    optimizer = optim.LBFGS([targetImage], lr = learning_rate)

    for k in range(1, steps+1):
        # Get feature maps from target image, then calculate the content loss
        print('Iteration [%d]/[%d]'%(steps,k))
        def cl():
            optimizer.zero_grad()
...
style_transfer(target,styleGrams,contentFeatures,steps = 150)
