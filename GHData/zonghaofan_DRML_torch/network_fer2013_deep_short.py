...
class Network_new(nn.Module):
    def __init__(self, class_number=6):
        ...
        self.extractor1_shortcut = nn.Sequential(
        ...
        self.extractor2_shortcut = nn.Sequential(
        ...
        self.extractor3_shortcut = nn.Sequential(
        ...
        #     nn.Linear(in_features=64 * 6 * 6, out_features=4096),
        #     nn.ReLU(inplace=True),
        #     nn.Dropout(0.5),
        #
...
if __name__ == '__main__':
    ...
    import sys
    print("hello world")
    image = Variable(torch.randn(2, 3, 48, 48))
    label = Variable(torch.from_numpy(np.random.randint(3, size=[2, 6]) - 1))
