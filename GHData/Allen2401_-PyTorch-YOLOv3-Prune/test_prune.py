...
class opt():
    ...
    model = 'checkpoints/darknetgood/checkpoint_85.pth'
if __name__ == '__main__':
    ...
    # %%
    # checkpoints / darknet / checkpoint_55.pth
    # 生成剪枝后的cfg文件并保存模型
    pruned_cfg_name = opt.model_def.replace('yolov3', f'yolov3_prune_{percent}_')
    pruned_cfg_file = write_cfg(pruned_cfg_name, [model.hyperparams.copy()] + compact_module_defs)
    print(f'Config file has been saved: {pruned_cfg_file}')

    compact_model_name = opt.model.replace('/checkpoint', f'/prune_{percent}_')
