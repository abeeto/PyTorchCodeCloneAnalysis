...
"""
Defines an API for Pybind Module of Torch Python API.
"""
import json
import os
import logging
...
class ModelHandler(object):
    ...
    def inference(self, input_data, input_shape):
        ...
        t = torch.Tensor(input_data[0]).reshape(input_shape[0])
        out =   self.model.forward(t)
        idx = torch.LongTensor([0, 1, 2, 3, 4])
        return out.index_select(1, idx).flatten().tolist()
