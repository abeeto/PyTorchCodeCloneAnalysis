...
# from data.bc_voc import VOCDetection
...
def parse_args():
    ...
    # Loss
    # --aux_loss 保存decoder中间循环的M层，最终对这M层进行nms
        ...
        if (epoch + 1) % args.eval_epoch == 0 or epoch + 1 == args.max_epoch:
            if evaluator is None:
                ...
                if distributed_utils.get_rank() == 0:
                    ...
                    print('cur_map > best_map:', cur_map > best_map)
                    if cur_map > best_map:
                        # update best-map
                        best_map = cur_map
                        # save model
...
#     if args.dataset == 'voc':
#         data_dir = os.path.join(args.data_root, 'BCdevkit')
#         num_classes = 1
