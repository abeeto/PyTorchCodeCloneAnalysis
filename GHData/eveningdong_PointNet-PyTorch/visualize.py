...
class Plot:
	...
	def draw_pc_semins(pc_xyz, pc_semins, fix_color_num=None):
		if fix_color_num is not None:
			ins_colors = Plot.random_colors(fix_color_num+1, seed=4)
		else:
...
if __name__ == '__main__':
	...
	parser.add_argument('--path', type=str, help='numpy file path')
	parser.add_argument('--label', type=str, default='', 
		help='numpy file path for the label')
    ...
    if args.label == '':
    	...
    	Plot.draw_pc_semins(data, label)
    else:
    	Plot.draw_pc(data)
