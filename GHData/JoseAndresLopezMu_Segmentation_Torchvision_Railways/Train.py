...
#TrainFolder="LabPics/LabPicsV1/Simple/Train/"
TrainFolder = "Railways_arranque/"
...
def ReadRandomImage(): # First lets load random image and  the corresponding annotation
    ...
    Filled =  cv2.imread(os.path.join(TrainFolder, "SegmentationClass_VIA", ListImages[idx].replace("jpg","png")),0)
    Vessel =  cv2.imread(os.path.join(TrainFolder, "SegmentationClass_FUERA_VIA", ListImages[idx].replace("jpg","png")),0)
...
Net = torchvision.models.segmentation.deeplabv3_resnet50(pretrained=True) # Load net #Mirar pesos
