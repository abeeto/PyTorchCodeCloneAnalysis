...
def compute_mu(flat_decision_p_e):
    batch_0_indices = torch.range(0, N_BATCH * N_LEAF - 1, N_LEAF).unsqueeze(1).repeat(1, N_LEAF).long()

    in_repeat = N_LEAF / 2
    out_repeat = N_BATCH

    batch_complement_indices = torch.LongTensor(
        np.array([[0] * in_repeat, [N_BATCH * N_LEAF] * in_repeat] * out_repeat).reshape(N_BATCH, N_LEAF))
    ...
    for i, flat_decision_p in enumerate(flat_decision_p_e):
        mu = torch.gather(flat_decision_p, 0, indices_var).view(N_BATCH, N_LEAF)
        mu_e.append(mu)
...
def compute_py_x(mu_e, leaf_p_e):
    ...
    for mu, leaf_p in zip(mu_e, leaf_p_e):
        py_x_tree = mu.unsqueeze(2).repeat(1, 1, N_LABEL).mul(leaf_p.unsqueeze(0).repeat(N_BATCH, 1, 1)).mean(1)
...
def update_leaf_p(mu_e, pi_e, py_x, Y_val, pi_update):
    ...
    for mu, pi in zip(mu_e, pi_e):
        ...
        for l in xrange(N_LEAF):
            for y in xrange(N_LABEL):
                indices = Y_val.eq(y).nonzero()
                
                if indices.nelement() == 0:
                    continue
                for i in xrange(len(indices)):

                    pi_update[l,y] = pi_update[l,y]+ pi[l,y] * mu[indices[i],l]/ py_x[indices[i], y]

        pi_update = pi_update.div(pi_update.sum(1).repeat(1, N_LABEL))
        pi_update_e.append(pi_update)
        pi_update.data.zero_()
    return pi_update_e
...
def train(model, loss, optimizer, X_val, Y_val, leaf_p_e):
    ...
    # compute mu
    mu_e = compute_mu(flat_decision_p_e)
...
# testing process
def predict(model, X_val, mode = 1):
    X_val = Variable(X_val)
    decision_p_e = model.forward(X_val)
    flat_decision_p_e = []

    for decision_p in decision_p_e:
        decision_p_comp = decision_p.unsqueeze(0).expand_as(decision_p) - decision_p
...
def main():
    ...
    for i in range(50):
        ...
        print("Epoch %d, cost = %f, acc = %.2f%%"
              % (i + 1, cost / num_batches, 0))#100. * np.mean(predY == teY[:2000])))
