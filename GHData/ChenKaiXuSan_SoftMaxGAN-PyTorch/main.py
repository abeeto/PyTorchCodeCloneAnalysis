...
from trainer import Trainer_softmaxgan
...
def get_parameters():
    ...
    parser.add_argument('--model', type=str, default='softmaxgan', choices=['softmaxgan'])
...
def main(config):
    ...
    if config.train:
        if config.model == 'softmaxgan':
            trainer = Trainer_softmaxgan(data_loader, config)
        trainer.train()
