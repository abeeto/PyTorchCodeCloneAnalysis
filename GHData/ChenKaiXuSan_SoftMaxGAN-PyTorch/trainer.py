...
# %%
class Trainer_softmaxgan(object):
    def __init__(self, data_loader, config):
        super(Trainer_softmaxgan, self).__init__()
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            ...
            for i, (real_images, _) in enumerate(self.data_loader):
                ...
                g_target = 1 / (self.batch_size * 2)
                d_target = 1 / self.batch_size
                ...
                # Partition function 
                Z = torch.sum(torch.exp(-d_out_real)) + torch.sum(torch.exp(-d_out_fake))

                # calculate loss of discriminator and update 
                d_loss = d_target * (torch.sum(d_out_real)) + self.log(Z)
            ...
            if (epoch) % self.log_step == 0:
                ...
                print("Elapsed [{}], G_step [{}/{}], D_step[{}/{}], d_loss: {:.4f}, g_loss: {:.4f}, "
                    .format(elapsed, epoch, self.epochs, epoch,
                            self.epochs, d_loss.item(), g_loss.item()))
