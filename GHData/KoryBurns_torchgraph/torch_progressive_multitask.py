...
from deepchem.models.torchgraph.torch_graph import TorchGraph, NNWrapper
from deepchem.models.torchgraph.layers import Layer, Feature, Label, Weights, \
    WeightedError, Dense, Dropout, WeightDecay, Reshape, SparseSoftMaxCrossEntropy, \
    L2Loss, ReduceSum, Concat, Stack, TorchWrapper, ReLU, Squeeze, SoftMax, Cast
from deepchem.models.torchgraph.IRV import Slice


class ProgressiveMultitaskRegressor(TorchGraph):
    ...
    for task in range(self.n_tasks):
      ...
      for i in range(n_layers):
        ...
        layer = Dense(
            ...
            activation_fn=None,
            weights_initializer=TorchWrapper(
                nn.init.normal_,
                std=self.weight_init_stddevs[i]),
            biases_initializer=TorchWrapper(
                nn.init.constant, value=self.bias_init_consts[i]))
        task_layers.append(layer)

        if i > 0 and task > 0:
          layer = layer + lateral_contrib
        assert self.activation_fns[i] is nn.ReLU, "Only ReLU is supported"
      ...
      layer = Dense(
          ...
          weights_initializer=TorchWrapper(
              nn.init.normal_,
              std=self.weight_init_stddevs[-1]),
          biases_initializer=TorchWrapper(
              nn.init.constant, value=self.bias_init_consts[-1]))
      task_layers.append(layer)
      ...
      label = Slice(task, dim=1, in_layers=[self._task_labels])
      weight = Slice(task, dim=1, in_layers=[self._task_weights])
      task_loss = self.create_loss(layer, label, weight)
      self.create_submodel(layers=task_layers, loss=task_loss, optimizer=None)

    outputs = Stack(dim=1, in_layers=outputs)
  ...
  def add_adapter(self, all_layers, task, layer_num):
    ...
    with self._get_tf("Graph").as_default():
      alpha = TorchWrapper(
          torch.autograd.Variable(
              nn.init.normal_((1,), std=alpha_init_stddev),
    ...
    dense1 = Dense(
        ...
        activation_fn=None,
        weights_initializer=TorchWrapper(
            nn.init.normal_, std=weight_init_stddev),
        biases_initializer=TorchWrapper(
            nn.init.constant_, value=bias_init_const))
...
class ProgressiveMultitaskClassifier(ProgressiveMultitaskRegressor):
  ...
  def create_loss(self, layer, label, weight):
    ...
    task_label = Cast(dtype=torch.int32, in_layers=[task_label])
