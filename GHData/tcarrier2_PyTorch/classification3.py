...
for i in range(0,N):
	if x[i,0]**2 + x[i,1]**2 < radius1**2:
		y[i] = 0
	elif x[i,0]**2 + x[i,1]**2 < radius2**2:
		y[i] = 1
	else:
		y[i] = 2
...
eq_y = radius1*np.sin(eq_x)
eq_x = radius1*np.cos(eq_x)
plt.plot(eq_x,eq_y)
eq_x = np.linspace(-10, 10, 1000)
eq_y = radius2*np.sin(eq_x)
eq_x = radius2*np.cos(eq_x)
plt.plot(eq_x,eq_y)
plt.axis([-4, 4, -4, 4])
plt.show()
