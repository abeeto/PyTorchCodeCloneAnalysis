...
# parser.add_argument("--image_folder", type=str, default="data/kaist/images/train/", help="path to dataset")
parser.add_argument("--batch_size", type=int, default=10, help="size of each image batch")
parser.add_argument("--model_config_path", type=str, default="config/yolov3-kaist.cfg", help="path to model config file")
parser.add_argument("--data_config_path", type=str, default="config/kaist_day.data", help="path to data config file")
...
print("Current time : {0} hours".format(start_time/3600))
model.train()
...
loss_data_file = open('loss data.txt','w+') #loss for each batch
...
for epoch in range(opt.epochs):
    ...
    for batch_i, (_, imgs, targets) in enumerate(dataloader):
        ...
        #     # viz.line(X=np.array([batch_i]), Y=np.array([losses_x]), win='loss', name='avg_train_loss', update='append')
        #     losses_x = losses_y = losses_w = losses_h = losses_conf = losses_cls = losses_recall = losses_precision = batch_loss= 0.0
        
        #imgs_num = 1
        #save the loss every batch
    ...
    if epoch % opt.checkpoint_interval == 0:
        model.save_weights("%s/%d.weights" % (opt.checkpoint_dir, epoch)) #save weight at every epoch
    ...
    #save epoch and plot
    print("Save loss function at epoch %d" % epoch)
    print("Time passed %.2f hours" % ((time.time()-start_time)/3600) )
...
print("Time : %.2f hours"% ((time.time()-start_time) / 3600) ) #seconds divided by 3600