...
class CocoDataLoader(object):
    def __init__(self, coco, mode='train'):
        self.coco = coco
        assert mode in ['train', 'valid'], 'Data loading mode is invalid.'
        self.mode = mode
    ...
    def get_img_annotation(self, ind=None, img_id=None):
        ...
        img_file = os.path.join(params['coco_dir'], self.mode, self.coco.loadImgs([img_id])[0]['file_name'])
...
if __name__ == '__main__':
    ...
    for mode in ['train', 'valid']:
        ...
        coco = COCO(os.path.join(params['coco_dir'], 'annotations/{}_baby_keypoints.json'.format(mode)))
        ...
        save_dir = os.path.join(params['coco_dir'], 'ignore_mask_{}'.format(mode))
