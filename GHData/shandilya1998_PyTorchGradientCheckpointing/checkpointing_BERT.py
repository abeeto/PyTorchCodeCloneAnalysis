# -*- coding: utf-8 -*-
"""checkpointing_BERT.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1Tew8-IDoIS1iCfFjwSFbNNNxYe0K7WY_
"""
...
def log_mem(model, inp1, inp2, mem_log=None, exp=None):
    mem_log = mem_log or []
    exp = exp or f'exp_{len(mem_log)}'
    hr = []
    for idx, module in enumerate(model.modules()):
        _add_memory_hooks(idx, module, mem_log, exp, hr)
    out, _ = model(inp1, inp2)
...
for i in tqdm(range(12)):
    for j in range(12):
        for k in range(12):
            if not (abs(i-j)<2 or  abs(k-j)<2 or abs(i-k)<2):
                model.set_ijk(i, j, k)
                exp = 'BERT_{i}_{j}_{k}'.format(i=i, j=j, k=k)
                if gpu:
                    tokens_tensor = tokens_tensor.cuda()
                    segments_tensors = segments_tensors.cuda()
                start = time.time()
                mem = log_mem(model, tokens_tensor, segments_tensors, mem_log, exp)
                df = pd.DataFrame(mem)
                res[exp] = df
                y = df['mem_all'].values.tolist()[:-2]
                x = df['call_idx'].values.tolist()[:-2]
                plots.append({'x':x,'y':y,'i':i,'j':j,'k':k,'exp':exp})

val = None
exp = None
for plot in plots:
    if max(y)<max_mem:
        val = (plot['i'], plot['j'],plot['k'])
        exp = plot['exp']
        max_mem = max(y)
...
exp_list = [[0, 2, 4], [0, 4, 8], [0, 3, 7], [3, 7, 10]]
mem_log = []
model = model.cuda()
if gpu:
    tokens_tensor = tokens_tensor.cuda()
...
val = []
exp = 'BERT_baseline'
wandb.init(project="assignment6", entity="shandilya1998", name = '{ex}'.format(ex = exp), id = '{ex}'.format(ex = exp))
mem = []
mem = log_mem(model, tokens_tensor, segments_tensors, mem, exp)
df = pd.DataFrame(mem)
res[exp] = df
y = df['mem_all'].values.tolist()[:-2]
x = df['call_idx'].values.tolist()[:-2]
...
k = 9
model.set_ijk(i, j, k)
model.set_embedding_ckpt(True)
exp = 'BERT_em_{i}_{j}'.format(i =i, j=j)
wandb.init(project="assignment6", entity="shandilya1998", name = '{ex}'.format(ex = exp), id = '{ex}'.format(ex = exp))
mem = []
mem = log_mem(model, tokens_tensor, segments_tensors, mem, exp)
[wandb.log(me) for me in mem]
...
res['BERT_baseline']['mem_all'].iloc[:-2].argmax()

res['BERT_baseline'].iloc[419]

str(model.encoder.layer[0])
