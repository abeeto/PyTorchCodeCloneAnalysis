...
from core.meta_sgd import MetaSGD
import copy
...
if __name__ == '__main__':
    ...
    meta_sgd = MetaSGD(copy.deepcopy(model))
    model_params = [p for p in model.parameters() if p.requires_grad]
    alpha_params = [p for p in meta_sgd.model.parameters() if p.requires_grad]

    model_optimizer = optim.Adam(model_params, args.beta)
    alpha_optimizer = optim.Adam(alpha_params, args.beta)
    ...
    for epoch in range(args.epochs):
        ...
        for support_images, support_labels, query_images, query_labels in train_bar:
            ...
            loss, acc = maml_train(model, support_images, support_labels, query_images, query_labels,
                                   1, model_optimizer, alpha_optimizer, meta_sgd)
        ...
        for support_images, support_labels, query_images, query_labels in val_loader:
            ...
            loss, acc = maml_train(model, support_images, support_labels, query_images, query_labels,
                                   3, model_optimizer, alpha_optimizer, meta_sgd, is_train=False)
