...
def get_traj(test_type, pa, env, episode_max_length, pg_resume=None, render=False):
    ...
    if test_type == 'PG':  # load trained parameters
        ...
        pg_learner = pg_network.Agent(pa)

        pg_learner.load_models()
    ...
    for _ in range(episode_max_length):
        ...
        if test_type == 'PG':
            a = pg_learner.q_eval.forward(T.tensor(ob)).argmax()
...
def launch(pa, pg_resume=None, render=False, plot=False, repre='compact', end='no_new_job'):
...
def main():
    ...
    launch(pa, pg_resume, render, plot, repre='compact', end='all_done')
