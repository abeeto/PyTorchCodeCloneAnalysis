...
class mAP_RetinaNet(RetinaNet):
    ...
    def detect_image(self,image_id,image):
        ...
        for i, c in enumerate(top_label):
            ...
            # f.write("%s %s %s %s %s %s\n" % (predicted_class, score[:6], str(float(left)), str(float(top)), str(float(right)),str(float(bottom))))
            f.write("%s %s %.2f %.2f %.2f %.2f\n" % (predicted_class, score[:6], float(left), float(top), float(right),float(bottom)))
...
print('average time is %.4fs' % (t_time / count_time))
print('average FPS: %.1fFPS' % (count_time / t_time))
