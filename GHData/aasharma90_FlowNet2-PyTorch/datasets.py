...
class StaticCenterCrop(object):
    ...
    def __call__(self, img):
        # - AA - return img[int((self.h-self.th)/2):int((self.h+self.th)/2), int((self.w-self.tw)/2):int((self.w+self.tw)/2),:]
        htmp_beg = int((self.h-self.th)/2);
        htmp_end = int((self.h+self.th)/2);
        wtmp_beg = int((self.w-self.tw)/2);
        wtmp_end = int((self.w+self.tw)/2);
        #print(htmp_beg, htmp_end, wtmp_beg, wtmp_end)
        return img[htmp_beg:htmp_end, wtmp_beg:wtmp_end,:]
...
class MpiSintel(data.Dataset):
    def __init__(self, args, is_cropped = False, root = '', dstype = 'clean', replicates = 1):
        ...
        #print('AA - flow_root', flow_root)
        #print('AA - image_root', image_root)

        file_list = sorted(glob(join(flow_root, '*/*.flo')))
        #print('AA - file_list', file_list)
        ...
        #print('AA - list', self.image_list)
        #print('AA - size_list=', self.size)
    ...
    def __getitem__(self, index):
        ...
        #print('AA - index', index)

        img1 = frame_utils.read_gen(self.image_list[index][0])
        img2 = frame_utils.read_gen(self.image_list[index][1])
        #imshow(img1), imshow(img2)
        #print('AA - flow_list[0]', self.flow_list[0])
