...
if __name__=="__main__":
    ...
    parser.add_argument('--model_path', type=str, default='./voc_81.6.pth')
    opt = parser.parse_args()
    model_path = opt.model_path
    eval_dataset = VOCDataset(root_dir='./VOC0712', resize_size=[800, 1333],
    ...
    model=RetinaNetDetector(mode="inference")
    # model=torch.nn.SyncBatchNorm.convert_sync_batchnorm(model)
    # print("INFO===>success convert BN to SyncBN")
    model = torch.nn.DataParallel(model)
    model.load_state_dict(torch.load('./checkpoint/voc_81.6.pth',map_location=torch.device('cpu')))
