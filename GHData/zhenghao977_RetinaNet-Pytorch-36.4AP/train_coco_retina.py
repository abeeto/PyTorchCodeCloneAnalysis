...
train_dataset=COCODataset("./data/train2017",
                          './data/instances_train2017.json',transform=transform)
...
for epoch in range(EPOCHS):
    for epoch_step,data in enumerate(train_loader):
        ...
        print("global_steps:%d epoch:%d steps:%d/%d cls_loss:%.4f reg_loss:%.4f cost_time:%dms lr=%.4e total_loss:%.4f"%\
            (GLOBAL_STEPS,epoch+1,epoch_step+1,steps_per_epoch,losses[0].mean(),losses[1].mean(),cost_time,lr, loss.mean()))
