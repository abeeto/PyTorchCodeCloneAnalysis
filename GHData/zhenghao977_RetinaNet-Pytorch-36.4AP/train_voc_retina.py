...
parser.add_argument("--lr", type=float, default=0.01, help="lr to use during batch generation")
...
train_dataset = VOCDataset(root_dir='.data/VOC0712',resize_size=[800,1333],
                           split='.data/VOC0712/trainval',use_difficult=False,is_train=True,augment=transform)

model = RetinaNetDetector(mode="training").cuda()
...
for epoch in range(EPOCHS):
    for epoch_step, data in enumerate(train_loader):
        ...
        print(
            "global_steps:%d epoch:%d steps:%d/%d cls_loss:%.4f reg_loss:%.4f cost_time:%dms lr=%.4e total_loss:%.4f" % \
