...
from cogan_pytorch.models.discriminator import discriminator_for_mnist
from cogan_pytorch.utils.common import AverageMeter
from cogan_pytorch.utils.common import ProgressMeter
from cogan_pytorch.utils.common import configure
from cogan_pytorch.utils.common import create_folder
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    inputs_dataset = torchvision.datasets.MNIST(root=args.data, download=True,
    ...
    if args.distributed:
        sampler = torch.utils.data.distributed.DistributedSampler(inputs_dataset)
    else:
        sampler = None

    inputs_dataloader = torch.utils.data.DataLoader(inputs_dataset,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        progress = ProgressMeter(num_batches=len(inputs_dataloader),
        ...
        for i, (inputs, target) in enumerate(zip(inputs_dataloader, target_dataloader)):
            # Move data to special device.
            if args.gpu is not None:
                inputs = inputs[0].type(torch.Tensor).expand(inputs[0].size(0), args.channels, args.image_size, args.image_size)
                inputs = inputs.cuda(args.gpu, non_blocking=True)
                target = target[0].cuda(args.gpu, non_blocking=True)
            ...
            # Train with real.
            inputs_output, target_output = discriminator(inputs, target)
            real_output = (inputs_output + target_output) / 2
            d_loss_real = adversarial_criterion(inputs_output, real_label) + adversarial_criterion(target_output, real_label)
            d_loss_real.backward()
            d_x = real_output.mean()

            # Train with fake.
            fake1, fake2 = generator(noise)
            inputs_output, target_output = discriminator(fake1.detach(), fake2.detach())
            fake_output = (inputs_output + target_output) / 2
            d_loss_fake = adversarial_criterion(inputs_output, fake_label) + adversarial_criterion(target_output, fake_label)
            ...
            inputs_output, target_output = discriminator(fake1, fake2)
            fake_output = (inputs_output + target_output) / 2
            g_loss = adversarial_criterion(inputs_output, real_label) + adversarial_criterion(target_output, real_label)
            g_loss.backward()
            ...
            iters = i + epoch * len(inputs_dataloader) + 1
        ...
        with torch.no_grad():
            fake1, fake2 = generator(base_image)
            fake = torch.cat([fake1.data, fake2.data], dim=0)
            vutils.save_image(fake.detach(), os.path.join("runs", f"GAN_epoch_{epoch}.png"), normalize=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--image-size", type=int, default=32,
                        help="Image size of high resolution image. (Default: 32)")
    parser.add_argument("--channels", type=int, default=3,
                        help="The number of channels of the image. (Default: 3)")
