...
from crf_encode_model import BioRnnDecoder, BioCNNEncoder, CMNBioCNNEncoder, BidRnnBioDecoder
from crf import CRF
from batch_getter import get_source_mask, BatchGetter, BioBatchGetter, CMNBioBatchGetter, get_target_mask
import random
import os
...
def bid_eval_all(config, my_arg, log_dir, pr=True):
    ...
    for i, this_batch in enumerate(batch_getter):
        top_path = crf_eval_one_sen(config, encoder, decoder, this_batch)
        # print [ner_tag.getWord(tag) for tag in top_path]
...
def cmn_crf_eval_all(config, log_dir, pr=True):
