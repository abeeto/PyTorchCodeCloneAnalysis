...
class DCGenerator(nn.Module):
    ...
    def forward(self, z, isTrain=True):
        ...
        if(isTrain):
            ...
            out = F.dropout2d(out) + torch.normal(mean=0, std=0.1*torch.ones(out.size())).to(out.get_device())
...
class CycleGenerator(nn.Module):
    ...
    """
    def __init__(self, conv_dim, init_zero_weights, device="cuda"):
        ...
        self.conv1 = conv(3,32,4,init_zero_weights=init_zero_weights)
        self.conv2 = conv(32,64,4,init_zero_weights=init_zero_weights)
    ...
    def forward(self, x, isTrain=True):
        ...
        if(isTrain):
            ...
            out = F.leaky_relu(self.resnet_block(out))
            out = F.dropout2d(out) + torch.normal(mean=0, std=0.1*torch.ones(out.size())).to(out.get_device())
