...
from PIL import Image   #python中最常被使用的图像处理包，功能强大， API简单
import argparse #好像后面没用到
import matplotlib.pyplot as plt
import torch
import torchvision
from torchvision import models #图像方面的几个模型，例如 AlexNet, VGG, and ResNet 以及 与训练好的参数。
from torchvision import transforms #常用的图像操作，例如：随机切割，旋转等。
...
num_workers = 2
dataset = torchvision.datasets.ImageFolder(root=dataroot,transforms=transforms.Compose([
        transforms.Resize(image_size),
        transforms.ConterCrop(image_size),
        transforms.ToTensor(),
