...
vocab = dict(Counter(text).most_common(MAX_VOCAR_SIZE-1))
...
class WordEmbeddingDataset(tud.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        neg_words = torch.multionmial(self.word_freqs,K*pos_words.shape[0],True)
...
class EmbeddingModel(nn.Module):
    def __init__(self,vocab_size,embed_size):
        ...
        self.out_embed.weight.data.uniform(-initrange,initrange)
        self.in_embed.weight.data.unifrom(-initrange,initrange)
...
for e in range(NUM_EPOCHS):
    ...
    if i%2000 ==0:
        ...
        sim_simlex = evaluate("simlem-999.txt",embedding_weights)
        sim_men = evaluate("men.txt",embedding_weights)
        sim_353 = evaluate("wordsim353.csv",embedding_weights)
        with open(LOG_FILE,"a") as fout:
            output = "epoch:{},iteration:{},\n simlex-999:{},\n men:{},\n sim353:{},\n nearest to monster : {}\n"
