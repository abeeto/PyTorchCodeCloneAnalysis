...
def test_net(name, net, imdb, max_per_image=300, thresh=0.05, visualize=False,
	...
	if i%1000 == 0:
        ...
        if visualize and np.random.rand()< 0.01:
            ...
            #cv2.waitKey(1)
	#    for myclass in xrange(1, imdb.num_classes+1):
	#	for mybbox in all_boxes[myclass][i].shape[0]
	#	    singlebox = tuple(all_boxes[myclass][i][mybbox,:])
	#	    cv2.rectangle(im2show, singlebox[:2], singlebox[2:4], (0,244,0), 2)
	    logger.image_summary('test/id{}'.format(i), im2show[None,:,:,-1::-1], step)
